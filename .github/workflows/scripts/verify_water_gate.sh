...
launch_once() {
  ...
  # Windowed + small so it never grabs the desktop; -verifyWater drives
  # WaterAcquisitionVerifyCapture; -logFile redirects the standalone player's Player.log so the
  # verdict lines are grep-able here. The component calls Application.Quit(0/1) when done.
  set +e
...
# self-assert failure is NOT a wedge — never retry it (it would mask a genuine loot/drink regression).
if [ "$exe_rc" -eq 124 ]; then
  echo "[verify_water] WARN — exe did not self-quit within ${LAUNCH_TIMEOUT}s (timeout-hang; likely the present-loop wedge) — retrying ONCE" >&2
  launch_once
fi
if [ "$exe_rc" -eq 124 ]; then
  echo "[verify_water] FAILED — exe did not self-quit within ${LAUNCH_TIMEOUT}s (hung launch, including the retry)" >&2
fi
