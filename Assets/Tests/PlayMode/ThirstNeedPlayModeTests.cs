...
namespace FarHorizon.PlayTests
{
    ...
    ///
    /// Proves the decay actually FIRES through Update over a REAL Time.time window — the complement to the
    /// deterministic EditMode math. This is the load-bearing headless guard: Time.deltaTime ~= 0 per frame in
    /// headless runs (unity-conventions.md §headless time), so a deltaTime-based decay would silently never
    /// tick. SurvivalNeed integrates over accumulated Time.time instead; this test confirms thirst measurably
    /// DROPS across a wall-clock window and that the drink-scoop seam restores it end-to-end. Mirrors
    /// HungerNeedPlayModeTests / WarmthNeedPlayModeTests.
    /// </summary>
    public class ThirstNeedPlayModeTests
    {
        ...
        public void SetUp()
        {
            ...
            _need.max = 100f;
            // Fast decay so the window stays short in CI but still spans real wall-clock seconds (NOT
            // per-frame) — 10/sec means ~2s of window yields an obvious, well-above-noise drop.
        ...
        [UnityTest]
        public IEnumerator DrinkScoop_Seam_RestoresThirst_AfterDecay()
        {
            yield return null;
            ...
            // The drink-action (FreshwaterPond) calls TryDrinkScoop with a proximity predicate. Here a true
            // predicate stands in for "the castaway is at the pond" — the seam contract is identical.
            bool drank = _need.TryDrinkScoop(() => true);
            Assert.IsTrue(drank, "drinking in range succeeds end-to-end");
