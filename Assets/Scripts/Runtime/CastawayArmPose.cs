...
namespace FarHorizon
{
    ...
    public class CastawayArmPose : MonoBehaviour
    {
        ...
        // (right wrist → −X world / left wrist → +X world — both OUTWARD, SAME sign), with near-zero raise. A
        // +rotation about LOCAL Z spreads further AND lifts the hand slightly (+Y) AND reaches it forward (the
        ...
        [Header("#3 — relax BOTH arms away from the torso (the pinched-idle fix)")]
        [Tooltip("Degrees of LOCAL-X rotation added to BOTH upper arms to spread them away from the torso a " +
                 "bit (less pinched). +X spreads BOTH arms OUTWARD (same sign — empirically measured), so NO " +
                 "mirror is applied; both arms get the same local-X relax.")]
        public float relaxSpreadDeg = 14f;

        [Header("#2 — right arm: a bit away from body + raised 10-15° (the held-axe ready carry)")]
        [Tooltip("EXTRA degrees of LOCAL-X spread added to the RIGHT upper arm only (on top of the relax).")]
        public float rightCarryExtraSpreadDeg = 6f;
        [Tooltip("Degrees of LOCAL-Z rotation on the RIGHT upper arm — empirically lifts the hand ~10-15° " +
                 "(+Y) AND reaches it forward, for the held-axe ready carry (#2 'raised 10-15°').")]
        public float rightCarryRaiseDeg = 16f;
        ...
        /// <summary>Rebuild the cached offset quaternions from the serialized degree values. Public so the
        /// editor author + any future nudge tool can re-apply after editing the fields.</summary>
        public void RebuildCached()
        {
            // BOTH arms: the relax is a +local-X spread (same sign — measured outward for both). The RIGHT
            // arm gets EXTRA local-X spread + a local-Z raise/reach (the carry). Compose X then Z in the
            // bone's local frame (order matters; X spread first, then Z lift on top).
            _leftOffsetQ = Quaternion.AngleAxis(relaxSpreadDeg, Vector3.right);
            _rightOffsetQ = Quaternion.AngleAxis(relaxSpreadDeg + rightCarryExtraSpreadDeg, Vector3.right) *
                            Quaternion.AngleAxis(rightCarryRaiseDeg, Vector3.forward);
        }
