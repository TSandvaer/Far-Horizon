...
0a2. **A small colour call Uma deliberately left to you.** `blender-asset-pipeline.md` §3 ships the weapon edge-highlight as `EdgeWhite #F5F5F0`. Erik's now-retired Rec 5 specified a **~2% warmer `#EBE6D6`**. She **declined to change it** — it would touch **15 shipped FBXs** — and wrote the reason into the doc so a future developer doesn't "fix" it unasked. Nothing is broken either way; this is a taste question about 2% warmth on every blade's highlight strip. Erik is being asked whether the specific value was load-bearing in his research or illustrative; if illustrative, this needs nothing from you at all.
