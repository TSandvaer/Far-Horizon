...
0a2. ✅ **RESOLVED before you saw it — no action needed.** Uma had queued a colour question (`EdgeWhite #F5F5F0` vs the ~2% warmer `#EBE6D6` from Erik's retired Rec 5, across 15 shipped FBXs). **Erik: the value was ILLUSTRATIVE, not load-bearing** — a worked example riding along with the retired mechanism, never soaked or board-validated. **No Sponsor time owed.** Uma's decision to leave `#F5F5F0` alone stands, and the doc already carries the reason so nobody "fixes" it unasked. Left recorded rather than deleted so the trail is visible.
...
1. **#351 SOAK — STAGED AND PLAYED. Run this:**
   ...
   **Full 9-step checklist is posted on the PR:** https://github.com/TSandvaer/Far-Horizon/pull/351#issuecomment-5137194461
   ...
   - 🔴 **THE SOAK QUESTION HAS CHANGED — bar #10 is NOT met, and the hand-off wrongly claimed it was.** Correction posted: https://github.com/TSandvaer/Far-Horizon/pull/351#issuecomment-5137465736
     - Drew's Predict-Before-Soak listed bar #10 (≥2 cue channels) as tested-and-satisfied, counting **float-bob** and **sway** as two channels. **Both are MOTION — that is ONE channel.** `team/quality-bars.md:33` is explicit: *"Colour-only is the most common way it fails, but **motion-only fails identically**"*, with FORM first, then POSITION, then MOTION. Raised by Uma reviewing #379; orchestrator verified against `origin/main` @ `e054aa7`.
     - **The bar exists because of this exact cue.** Its own *Second motivating instance* (Devon, PR #349 review, 2026-07-27): *"a find-in-world attract cue lost its Fresnel rim and now rests on motion alone — the earlier colour-only wording of this bar would have PASSED it, which is why the invariant is single-channel collapse, not colour-only."*
     - **So Drew's played finding is the PREDICTED symptom, not a tuning nit.** He reported that at default overhead framing the sword did **not** draw his eye and only read clearly once he lowered the camera. Bar #10 had already ruled that class a defect before this build existed.
     - **Your decision is therefore a design call, not an eye test:**
       **(a)** add a channel that **VARIES PER INSTANCE** — Drew's substitutes, now in #379: a **per-instance marker mesh** (FORM), or **reserved placement** (POSITION); colour ranked last per bar #10. **or (b)** waive bar #10 for this cue as a deliberate, recorded exception.
       - ⚠ *Corrected twice. My first version of option (a) named the **white edge-highlight plane** (`blender-asset-pipeline.md` §3), repeating Uma's prescription. **Both were wrong.** The plane IS fork-free — `EdgeWhite #F5F5F0` is a shared-palette slot — but **§11 mandates it on every blade** and the palette row scopes it to all weapons, so it is **invariant** and cannot discriminate the pickable item. It would leave the cue collapsed on MOTION by a different route. Drew found this only because he was asked to VERIFY the prescription rather than apply it. Correction posted: https://github.com/TSandvaer/Far-Horizon/pull/351#issuecomment-5137524886*
       - **The generalisable test, worth keeping:** *a channel that is always on is not a cue.* The edge highlight passes "fork-free?" and passes "is it FORM?" and still fails, because it does not vary.
       - 📌 **Bar #10 has been AMENDED because of this, and it changes what your decision means.** PR #380 (Priya, in review by Tess) sharpens the *definition* of "channel" rather than adding a clause: *"a property that DIFFERS between an instance in the cued state and one that is not — present on every instance is **style, not a cue**. Variance is the precondition for being a channel at all."* Plus a desk check before the desaturate test: name what the channel looks like on a NON-cued instance; if "the same", strike it and re-count. FORM→POSITION→MOTION ranking unchanged (read-speed is orthogonal to variance) and the bar now says so. Recorded as the bar's **Third motivating instance**.
         - So option **(b)** is no longer "waive a bar someone wrote down" — it is a recorded exception to a bar that now has three motivating instances and a mechanical check. That is a stronger thing to waive, and worth knowing before you pick.
         - 🔴 **Tess APPROVED #380 — and broke it three ways. One of them lands directly on YOUR #351 decision.** (Verdict 5137658173; filed as `86caz5na6`; she rated the amendment *"correct and better; incomplete, not wrong"*, so #380 still merges after a one-clause N1 fix.)
           - **Her root cause:** every bar-#10 check runs on the **cued instance alone**, so the bar measures **presence, never discrimination**. Her version history is exact — *"v1 counted channel types, v2 counted information, v3 counts variant information; none asks how much."*
           - **Evasion 1 (primary):** variance is a **boolean, never a magnitude**. A **2 cm marker pebble + 3 mm bob** passes every clause and is invisible at gameplay camera. So "add a per-instance FORM channel" — option (a) — can be satisfied and still not work. If you pick (a), the amplitude is the thing to judge, not the presence.
           - **Evasion 2:** two counted channels can share one failure domain — a single marker mesh is both FORM and POSITION, so one null ref kills both silently.
           - **Evasion 3 — ⚠ NOW OVERTURNED, and it was MY framing that was wrong.** I told you the bar *"returns no verdict at all"* on #351 because it ships one `sword_iron` at the default dial. Tess disproved that in PR #386: step 1 IS genuinely empty (verified — `DefaultFindCount 1`, `ApplyActiveCount` `SetActive(false)`s the rest), **but step 2 is populated and the author already named it from a capture** — the rust-capped scatter rocks + chop-tree trunks. Her words: *"The bar was never silent on #351; step 2 was unwritten."* It had already discriminated once, at proud height **0.34→0.60 u**. **So option (b) is NOT waiving an inapplicable bar — the bar does apply and does have a comparison.** That makes (b) a heavier choice than I described.
           - **Her proposed real fix:** replace the author-sentence desk check with an **artifact** — a two-sided capture with cued and non-cued instance **in the same frame**, and a naive viewer points at the cued one. That makes discrimination the measured thing, objectively, the way the desaturate test is objective.
         - ⚠ *Orchestrator correction: I described the bar's Third motivating instance to Priya as coming from a "played build". Tess caught that it overclaims — #351 is OPEN with no Sponsor verdict (served `23:09Z`, corrected `23:50Z`). Priya's "not soak-confirmed" rating was right; my framing was wrong. Her wording stands unchanged.*
     - **The rim analysis stands and is verified** (`WeaponPackAssetGen.cs:273` → URP/Unlit; zero rim properties on the shared material; setting one is a silent no-op). The rim really is unreachable without forking — which is *why* the fix is a FORM channel rather than a shader channel.
     - Everything else in the hand-off is unaffected: steps 1–2, 4–9, the exe path, the stamp, all four green CI jobs.
...
1b. **Original entry — every machine gate is clear; you are the only thing left.** Drew un-conflicted it (head `b1560cf`) and run `30586659470` is **`structure` + `build` + `capture` all SUCCESS** (playmode cancelled — advisory, and main's own run shows the same shape, so pre-existing). Drew is staging the exe from the CI artifact into `Build/soak-351/` with a played-verification and a keys-named checklist; the exe path + verbatim HUD stamp will be appended here when he reports.
   ...
   - **How #351 nearly shipped broken, worth knowing before you judge it:** git merged `test_gate_scripts.sh` **cleanly** while leaving #351's new capture gate **unregistered** in both `HEADLESS_GATES` and `WINDOWED_GATES` — a clean auto-merge is NOT evidence a gate is wired. Caught at `170 pass / 1 fail` **as reported by Drew from his own run** — ⚠ Devon could NOT independently verify that count (the mutated tree is gone) and replaced it in PR #383 with a git-measurable fact (`43ddeee`/`259d890`). Treat `170p/1f` as a reported observation, not an established measurement. Fixed to `176/0`.
