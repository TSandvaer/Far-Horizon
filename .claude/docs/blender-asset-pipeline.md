...
Write the `bpy` operations (material setup, UV placement, transform-apply, normals, FBX export) as a standalone `.py` script and pass it via `--python`. **The `bpy` API is identical** — every snippet that would run in `execute_blender_code` runs unchanged in the script; every YES/PARTIAL row in the table above is achievable this way. Exit code 0 = success; non-zero = a Python exception (read stdout for the traceback). **Commit the script under `tools/debug/` with a one-line row in `tools/debug/REGISTRY.md`** so future passes re-run deterministically — *not* alongside the exported FBX, and *not* beside the `.blend`. Two locations, two artifact kinds: **`.blend` modeling sources / palette PNGs / concept art / hero+family renders / provenance `README`s → `art-src/`** (`:24` above), **executable Blender `.py` → `tools/debug/` + a `REGISTRY.md` row**. Measured on `origin/main` 2026-07-30: all **28** committed `.py` live under `tools/debug/`, and `art-src/` + `Assets/Art/` hold **zero** (`86caz4td5` — this sentence previously sent the script to a directory nothing is committed to; the withdrawn wording is quoted in `team/DECISIONS.md`'s 2026-07-30 CORRECTION entry, deliberately not here, so a staleness grep for the old phrase stays clean).
