...
- ⚠ **"Per-prop instances opt in" does NOT mean every prop can.** `_RimIntensity` is declared in exactly one place — `Assets/Shaders/LowPolyVertexColor.shader` — so **only objects already rendering with that shader can opt in.** Verified 2026-07-27 against `origin/main`: the whole weapon/tool family renders on `Mat_WeaponPalette`, which `WeaponPackAssetGen.cs:273` builds from `Shader.Find("Universal Render Pipeline/Unlit")`. **Weapons, tools and any other URP/Unlit-material prop therefore CANNOT take a rim, and no amount of material tweaking gets one** — it needs a different shader.
- **Consequence for spec authors:** do not write a Fresnel-rim attract cue, hover glow, or silhouette highlight into a ticket for a weapon/tool without first checking which shader the target actually renders with. This already bit once — `86cah7y5b` AC3 specced "gentle float-bob + a low Fresnel rim via the existing `_RimIntensity` opt-in" as the find-in-world attract cue, and the rim half was unreachable as written; the implementer had to ship bob-only rather than smuggle a second material into the PR. The one-material / ~1-SetPass discipline (`WeaponPackAssetGen.cs:12,29`) is the thing being protected, so "just add the rim shader to weapons" is the wrong fix, not the obvious one.
- ⚠ **"The R5 pin was removed" does NOT weaken the bullet above — it is a DIFFERENT mechanism, and conflating the two nearly reopened a settled scope call (2026-07-28).** A reviewer read `MovementCameraScene.cs:1147` — *"R5 (86cahne3d): the URP/Unlit pin is removed — the shared Mat_WeaponPalette ships as a serialized material reference (its shader ships with it)"* — as evidence that `Mat_WeaponPalette` may no longer be URP/Unlit, and therefore that the rim might be reachable after all. It is not evidence of that. R5's own regression guard settles what "pin" means: `ZoneDLookTests.cs:345-388` (`R5_UrpLitAndUnlit_NotPinnedAlwaysIncluded_FourFarHorizonShadersRemain`) asserts only that URP/Lit + URP/Unlit are absent from `GraphicsSettings.m_AlwaysIncludedShaders`, and its docstring says outright: *"They still SHIP — via the many serialized material references in Boot.unity — so the look is unchanged."* **`m_AlwaysIncludedShaders` is a build-STRIPPING registration; it has nothing to do with which shader a `.mat` is assigned.** Verified on `origin/main` @ `fee2604`. Two independent facts on the same commit still point the original way: `WeaponPackAssetGen.CreateOrUpdateMaterial()` re-assigns `mat.shader = unlit;` **unconditionally** in its existing-material branch on every bootstrap bake (so a fresh bake always lands URP/Unlit), and CI re-bakes before EditMode + build. ✅ **RESOLVED same day by the decisive read — the bullet above STANDS, weapons genuinely cannot take a rim.** `Assets/Art/Props/WeaponPack/Mat_WeaponPalette.mat:24` on `origin/main` carries `m_Shader: {fileID: 4800000, guid: 650dd9526735d5b46b79224bc6e94025}`, and `Library/PackageCache/com.unity.render-pipelines.universal@*/Shaders/Unlit.shader.meta` carries **that exact guid** — so the shipped material is URP/Unlit, which declares no `_Rim*` property at all. Confirmed twice independently (orch-side GUID match + Drew's read on ticket `86cah7y5b`, where a `SetFloat` was verified to be a silent no-op). **The reviewer's "the pin was removed, so maybe it can" reading was wrong for the reason above — `AlwaysIncludedShaders` is not material assignment.** Kept written down rather than deleted, because the wrong inference is the reusable lesson: two true statements about different mechanisms nearly reopened a settled scope call.
- **The general rule this produced:** a shipped material's shader is proved by the `.mat`, not by prose about shaders elsewhere in the codebase. Three distinct facts share loose "pin / shader / material" vocabulary — the `AlwaysIncludedShaders` build-stripping registration, a generator's `Shader.Find` / `mat.shader =` assignment, and the asset's own serialized `m_Shader` — and a comment about any one of them is not evidence about the other two. See `unity-conventions.md` §Build stripping & shaders.
