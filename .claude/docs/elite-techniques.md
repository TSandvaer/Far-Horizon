...
- **Populated-island scaling checkpoint (island C4, PR #278 — cite the PR as primary; backing analysis `team/analysis/2026-07-07-island2-c4-perf.md` lives on the PR branch until #278 merges): the FINAL POPULATED 600u-scaled (~1200u-diameter) POC island held 60fps (vSync-capped) in the shipped exe, and 2.05 ms/frame (~488fps) UNCAPPED** — 56 draw calls / 0.94M tris, shadow casters 35/42 with vegetation `castShadows:false`. This extends the ~800u single-mesh checkpoint above to a FULLY-POPULATED island at the next size tier and adds the real uncapped-GPU headroom number the prior bullet flagged as missing — **chunked/streamed terrain (Lague) is STILL not required at ~1200u diameter.** The vegetation `castShadows:false` split (35 casters vs 42 renderers) is the load-bearing lever holding the shadow-pass cost down at this population density — carry it forward when scaling to the next tier.
