...
> **Weapon/tool FAMILY-COHESION is a STYLE-SYSTEM decision, not per-asset sourcing (Route A, Sponsor 2026-06-19).** The hand-tool/weapon family (axe, knife, sword, spear, …) reads as ONE family only if it shares ONE style spec + ONE shading model + ONE shared low-poly palette material through ONE in-house Blender pipeline — NOT by sourcing each item separately (which is what produced the outlier axe with its baked photographic atlas). The anchor is the spec (`team/uma-ux/weapon-tool-style-spec.md`, Uma owns) + the world palette extracted from the live build. **DONE — the re-make shipped 2026-06-23 in PR #100 (`031d43a`, ticket `86cabh907`).** The CC-BY axe that prompted this decision (`CastawayAxe/`, Viktor.G) was a **PLACEHOLDER, never the style anchor**, and it is gone — deleted with its CC-BY license file in that PR, which retired the attribution obligation with it. The live weapon/tool family is `Assets/Art/Props/WeaponPack/` (shared palette material); the hero axe is `wpn_axe_stone_01`. The rationale stands as the standing rule: **route new items through the one in-house pipeline; do not source a one-off.** (DECISIONS 2026-06-19; rationale memory `weapon-tool-unified-style-inhouse-blender-set`.)
