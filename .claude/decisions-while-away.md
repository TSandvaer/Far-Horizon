...
## 2026-07-31 0410 UTC — Label-merge PR #380 (bar #10 invariance clause), delta confirmed

- **Decided:** label-merge PR #380 (head `2352623`, docs-only, `docs-markup` SUCCESS, `MERGEABLE`).
- **Foundation:** `classifier-blocks-merge-to-protected-main` standing auto-merge. **Tess APPROVE** at `49ce15f4`, then **CONFIRM** on the post-approve delta at `2352623` (comments 5137658173 + 5137747272). The re-confirm was run because an APPROVE covers only the sha it read and Priya explicitly disclosed the additions as post-approve prose — she flagged the gap rather than letting it ride.
- **What the amendment does:** sharpens the *definition* of "channel" instead of adding a clause — a channel is a property that DIFFERS between a cued and a non-cued instance; present on every instance is **style, not a cue**. Variance is the precondition for being a channel at all. Plus a desk check before the desaturate test, and the FORM→POSITION→MOTION ranking left explicitly unchanged (read-speed is orthogonal to variance).
- **Merged KNOWN-INCOMPLETE, deliberately.** Tess broke the amendment three ways (filed `86caz5na6`): variance is tested as a boolean and never a magnitude (a 2 cm pebble + 3 mm bob passes and is invisible); two counted channels can share one failure domain; and the check returns NO VERDICT on a unique instance — which is exactly #351 at the default dial. She rated it *"correct and better; incomplete, not wrong"*, and the merged bar leads its pointer with `KNOWN INCOMPLETE`, magnitude evasion first, conceding it cannot adjudicate #351. A bar that read as finished while its own reviewer had enumerated three evasions would have been the worse outcome.
...
## 2026-07-31 0300 UTC — CORRECTION: "docs-only ⇒ zero checks" is STALE, and I merged the workflow that made it stale

- **The error:** I wrote *"docs-only → `paths-ignore` means ZERO CI checks; that is nothing-ran, not green"* into brief after brief, into ~5 ticket bodies, and into several merge-decision log entries tonight. **It is wrong.** Caught by Priya in her #384 hygiene pass; verified by me immediately after.
- **Ground truth:** PRs #381 / #382 / #383 / #384 / #385 each show **`docs-markup (hosted, no license): SUCCESS`** — a real, passing check. `.github/workflows/docs-markup.yml` exists on `main` and its own header states why: `ci.yml`'s workflow-level `paths-ignore` skips the WHOLE run for a docs-only PR, so `structure` never fires; this separate workflow closes exactly that gap.
- **The part worth remembering:** that workflow is the one **I merged tonight as #365** — the PR the Sponsor pre-approved. So I merged the fix and then spent hours describing the world as though it had not landed. The stale premise survived because I kept restating it from memory instead of reading `gh pr checks` on the PRs in front of me. Same root cause as the other lagging-read errors this session.
- **Corrected wording going forward:** a docs-only PR gets **exactly one real check, `docs-markup`, and it is a genuine green.** It does NOT get `structure`/`build`/`capture`/`playmode`, and their absence is expected rather than a gap.
- **Priya's board resolutions, applied by me** (she made zero ClickUp writes by design):
  - `86caz5na6` → **`in progress`** (Tess is dispatched on it; it was showing `to do`).
  - `86cayp1w2` / `86caywf84` / `86cavj6p1` → **retitled with a `[ABSORBED into 86caz42tv - do not dispatch]` prefix, status deliberately left `to do`.** Her reasoning, which is better than my earlier instinct: a **title token is what a board scan greps**, while `complete` would assert work that has not happened. Step two is to flip all three to `complete` in the same round as `86caz42tv`'s merge, when `complete` becomes true. She also said plainly that her earlier close-as-duplicate recommendation *"was under-specified and that's what stalled it"* — I had declined it, and the resolution is better for the round-trip.
- **Her merge recommendation, not yet acted on:** fold `86caz4wwx` into `86caz4yhj` — **both AC3s build the same guard, and that guard ALREADY EXISTS as `docs-markup.yml`**, which answers `86caz4yhj`'s AC4 outright and drops it a size class. Left for the next tick rather than executed inside this correction entry.
- **Also flagged by her:** `86caz428q`'s urgency premise is stale post-#369; `86caz5nr2` should sequence FIRST because four other tickets re-derive its rule; `86caz4mpy` + `86caz4mq8` stay split but share ONE soak; and `team/priya-pl/decisions-batch-pr-template.md` referenced in `.claude/agents/priya.md` **does not exist** — a Godot-era carry.
- **Status:** pending review

## 2026-07-31 0330 UTC — Label-merge PR #382 (Rec 5 retirement); and a ticket I filed on a WRONG recommendation

- **Decided:** label-merge PR #382 (`86caz5m75`, head `e870628`, `MERGEABLE`, `docs-markup` SUCCESS).
- **Foundation:** standing auto-merge for fully-gated non-soak PRs. **Devon APPROVE** (comment 5137960894), with independent measurement rather than acceptance: both dating shas hold and are **stronger** than Uma claimed — `b5d530c` 2026-06-17 23:13, `4b15255` 2026-06-17 00:21, `c7658b3` 2026-06-19 22:21, gap **47h08m**, and each is the FIRST commit touching its file so `blender-asset-pipeline.md` did not exist when Rec 5 landed. Rec 5 also survived 3 later edits untouched. Both of Uma's AC3 measurements reproduced exactly (Rec 6 grep → 0; `UnlitForwardPass.hlsl` `Attributes` = position + uv only, `grep -c COLOR` → 0). Rec 2 clean confirmed. #379's style-vs-cue distinction preserved verbatim at `:86`/`:405`.
- **⚠ The important part is a ticket I got wrong.** I filed `86caz5pc3` with *"narrow Rec 6 to Lit-shader procedural use"* as its **load-bearing instruction**, taken from Erik's intent review and relayed as authoritative **without measuring it**. Devon measured: **URP/Lit ALSO has no `COLOR` semantic** (0 across `LitForwardPass`/`GBuffer`/`Input`), so Lit is not the axis at all — the shader that consumes vertex colour is `FarHorizon/LowPolyVertexColor` (`:173`/`:308`). And `CraftStumpMat.mat` is URP/Lit, so **the stump half is dead too**, which Erik's enumeration assumed was fine.
- **Corrected by comment on `86caz5pc3` BEFORE the merge**, with a corrected AC1 (name the shader, not the lighting model), an AC2 addendum (both Unlit and Lit lack `COLOR` — the mechanism matters, because a reader who thinks Lit is the escape hatch repeats this exact error), and a new AC5 (re-check every remaining asset against its actual shader). The ticket is now flagged **do not dispatch until fixed**.
- **The asymmetry worth remembering:** the dating/grep facts are double-measured (Uma, then Devon). The narrow-to-Lit recommendation was **single-sourced from reasoning over pasted values with no repo read** — Erik had none available by design ([[erik-agent-no-bash-and-erik-wt-missing]]). I treated a reasoned recommendation as equivalent to a measured one. It is not, and the routing rule I wrote hours earlier says so.
- **Also noted:** #382's PR body repeats my stale *"ZERO checks"* wording; `docs-markup` ran SUCCESS. The premise is corrected in the log and in all briefs since, but it propagated into an agent's PR body before I caught it.
- **Status:** pending review

## 2026-07-31 0345 UTC — Merge-order constraint discovered: #381 MUST land before #386

- **Found by Priya** in her convention review of #386 (comment 5137968846, `APPROVE_WITH_NITS`, convention half only — Devon still owes the maths half).
- **The constraint:** #386 cites `team/erik-consult/two-sided-capture-feasibility.md`, which is **absent from `main`** — it exists only on PR **#381**'s head, which is still OPEN. Per the committed-artifact rule, a spec may not cite a note that has not merged. **So #381 must merge before #386, or #386 must mark the path pending.**
- **Consequence for the queue:** #381 is currently under review by Uma (part of her #384/#381/#385 batch). #386 therefore waits on TWO things, not one — Uma's #381 verdict *then* the merge, plus Devon's maths review. Recorded so a later tick does not merge #386 into a dangling citation.
- **Her measured answer to the question I asked** (is the row too heavy?): **yes, and it already was before #386.** Bar 10's cell is **2,523 chars / 6,857 for the whole row**, against bar 9's **391 / 511** — a **13.4×** outlier. Proposed shape: keep ONE bar, cut the cell to a one-line standard + `see § Bar 10` under a ~400-char house ceiling, and move the three motivating instances into the appendix **#386 itself creates**. She deliberately did NOT push that diet into #386, on the grounds that **the bloat arrived with her own `90d024b`** and is hers to file — a clean separation of concerns rather than expanding someone else's PR.
- **Bar #10 DOES satisfy its own `## How to use` line** (the methodology line I suggested and Tess confirmed): all four checks state what they return on an instance that should FAIL, each with a constructed failing instance and a stated verdict, and the preamble cites the rule by name. **She notes it is the ONLY row in the file that does** — which reframes the line from "does this row comply" to "the rest of the file now doesn't".
- **Three substantive NITs she raised, all worth keeping:** (1) the status clause **over-reads** "no longer KNOWN-INCOMPLETE" — C1's rule is live but its judge and floor are not, so it should be stated per-check; (2) the section titled "the four checks" **omits desaturate**, which is still required; (3) "precedent for C3" should read **"author-side precedent"** — she traced the draft-2 rejection to a developer code comment at `b1560cf`, **not** a Sponsor verdict. That last one is provenance discipline catching a quiet upgrade, the same class I committed twice tonight.
- **Status:** pending review
