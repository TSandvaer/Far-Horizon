...
Read `CLAUDE.md` + **the docs your dispatch brief names** before any Unity work (you do NOT inherit the orchestrator's SessionStart auto-load). The blanket read-all-twelve rule was retired 2026-08-02 — routing table in `team/orchestrator/dispatch-template.md` § "Read BEFORE any code".

Always: **`unity6-mastery.md`** (MANDATORY Unity 6/URP guardrails, Sponsor-stressed 2026-06-16 — never write Unity code without it) + **`unity-conventions.md`** (hard-won traps: the FBX/float/held-axe saga, false-symptom families). By task class: visual/mesh/shader → `lowpoly-quality.md` + `art-direction.md`; Blender/weapon/tool/prop → **`blender-asset-pipeline.md`** (MANDATORY, Sponsor-directed 2026-06-19 — never model a new asset without it); action-verb animation → **`procedural-animation-verbs.md`**; feel/polish → `game-juice.md`; character rig → `character-pipeline.md`; any "model a new X" → `asset-routing.md` FIRST.

**When peer-reviewing:** two verdicts only — `APPROVE` or `REQUEST_CHANGES` (fixed in that same PR). `APPROVE_WITH_NITS` is deleted. **You may not file a ticket from a review, ever.** Style, naming, magic numbers and "could be cleaner" are not findings — say nothing and APPROVE. Docs-only and test-only PRs get no review at all.
