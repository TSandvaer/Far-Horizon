...
- [ ] **Ticket ID + body included verbatim** in the brief — they read the ticket body from the brief, not from the board. (Citation corrected 2026-08-21: the memory `sub-agent-mcp-tool-surface-scope` **does not exist**. The real one is `subagents-cannot-reach-clickup-mcp`, 2026-07-01, which carries its own 2026-07-06 reversal — ClickUp reach working again — and has not been re-tested since. So sub-agent ClickUp reach is **UNVERIFIED, not absent**. Inline the body regardless: it costs nothing and removes the dependency either way.)
...
- **MCP-driven asset gen (PixelLab / pixel-mcp / bgclear.ai / Blender MCP)** runs in the orch main session, not Uma sub-agent dispatch — but **not for the reason this line used to give.** The retired claim ("non-clickup MCP tools don't inherit to sub-agents", citing a memory `sub-agent-mcp-tool-surface-scope` that does not exist) was **disproven by measurement 2026-08-21**: a `general-purpose` sub-agent (`tools: *`) loaded `mcp__blender__get_addon_status` via `ToolSearch` and invoked it, receiving the same server response as the orchestrator's own baseline call in the same session. **Non-ClickUp MCP tools DO reach sub-agents.** The real constraint is narrower: the six named personas enumerate explicit `tools:` lists containing only `mcp__clickup__*`, so *they* cannot reach other MCP servers — their own allowlist, not an inheritance rule. Adding an MCP tool to a persona's `tools:` line would grant it (mind `agent-file-crlf-colon-registration-trap.md` before editing that frontmatter). Whether to do so for the Blender lane is an **open Sponsor decision** — the 2026-08-18 CORRECTION in `team/DECISIONS.md` left it "untested, not disproven"; it is now tested. Until that decision, Uma writes the prompt + the orchestrator executes the gen.
