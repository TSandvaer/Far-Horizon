...
2. **Priya** — collects `Decision draft:` lines from merged PRs each Monday. Batches them into a single weekly PR (title: `pm(decisions): weekly batch — YYYY-MM-DD`). The entry shape and the amend rules come from `team/DECISIONS.md`'s own header: its **Append protocol** blockquote, the fenced entry-format block under it, and its **Correction protocol** blockquote. There is no separate batch-template file and none is owed — a second copy of the protocol is a second thing to drift, and both batches shipped to date (through 2026-07-31) were shaped from that header.
3. **Orchestrator** — may append urgent cross-role calls or Sponsor directives directly (in-session only, when the decision is blocking and cannot wait for the next batch). Must still use PR-flow; no force-push to main. **Urgency changes the timing, not the entry:** an entry appended on this route takes its shape and its amend rules from the same source item 2 names — `team/DECISIONS.md`'s **Append protocol** blockquote, the fenced block under `Append-only. Format:`, and its **Correction protocol** blockquote. Blocking is a reason to skip the batch; it is never a reason to skip the fields.
