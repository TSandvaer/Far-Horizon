...
|---|---|---|---|
| **AXE** | the SHIPPED in-hand axe — `Assets/Art/Props/WeaponPack/wpn_axe_stone_01.fbx` (`WeaponPackAssetGen.AxeFbxPath`), the same FBX `MovementCameraScene` seats on the right-hand bone. **⚠ CORRECTED 2026-07-29: this cell used to name `CastawayAxe.fbx`, deleted in PR #100 (`031d43a`, `86cabh907`) when the in-house weapon SET shipped — that path does not exist, do not bake from it.** | 3/4, head up-left, haft trailing down-right (reads as an axe in silhouette instantly — matches `21h08_08`'s read) | the shipped prop's OWN surface — the stone-tier recipe measured in [`weapon-tool-style-spec.md`](weapon-tool-style-spec.md) §4.1 (knapped grey biface `#8E8A82` + white cutting-arc band `#E4E2DC` + straight `#7A5230` haft with a `#5A3B22` grip band). **⚠ CORRECTED 2026-07-29: this cell used to read "the SHIPPED atlas — slate/steel head, warm-brown haft, dark leather-wrapped grip". That described the RETIRED CC-BY hatchet: §4.1 states the shipped stone axe has "NO lashing, NO rawhide, NO cord", and a per-asset baked atlas is banned outright by [`blender-asset-pipeline.md`](../../.claude/docs/blender-asset-pipeline.md) §0 — bake the mesh's shared-palette material, never an atlas.** **Do NOT recolor the icon to barn-red** — the shipped held axe is the accepted grey knapped-stone head (DECISIONS 2026-06-14: "axe-head ACCEPTED… genuinely looks like an axe"; barn-red recolor DROPPED, and the in-house stone axe that replaced it kept the grey). The icon must match the in-hand axe the player sees, not the abstract style-guide barn-red ideal. Icon ⇄ held-prop consistency wins. **Operative bake spec: [`item-icon-bake-recipe.md`](item-icon-bake-recipe.md) §2.1** (this table is the parent; that doc supersedes-in-detail). |
...
- `Assets/Art/Props/WeaponPack/wpn_axe_stone_01.fbx` — the shipped in-house stone axe FBX (the AXE icon's render source; surface per `weapon-tool-style-spec.md` §4.1, NOT barn-red per DECISIONS 2026-06-14). **Replaces `CastawayAxe/`, deleted in PR #100 (`031d43a`).**
