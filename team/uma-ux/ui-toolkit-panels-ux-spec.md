...
> **Staleness-scan note (`86cayw6ve`).** Exactly **two** lines in this file legitimately contain the RETIRED axe descriptor: §4.6's `⚠ CORRECTED` marker (which quotes the wording in order to retire it) and the DECISIONS citation directly above. A bare "zero hits" grep for that phrase therefore can NEVER pass, and must never be "satisfied" by striking either line — that would destroy the record the marker and the citation exist to keep. The keepable form negates the marker context (`… | grep -v 'CORRECTED\|DECISIONS'` → **zero LINES**; judge the OUTPUT, not the exit code — a clean run exits 1). This note deliberately does not quote the phrase itself, so the count it states stays true. Exact command, the general rule, and the prove-the-RED requirement: [`team/TESTING_BAR.md`](../TESTING_BAR.md) § "Doc-staleness greps".
</content>
</invoke>
