...
NOT re-decided here — `.slot__icon` is an `Image`/`VisualElement` whose `backgroundImage` is the `IconBaker`-rendered sprite of the actual prop (axe = the shipped `Assets/Art/Props/WeaponPack/wpn_axe_stone_01.fbx`; the wood + stone RESOURCE items = Blender-MCP props). **"wood/stone" here names the two gathered MATERIALS, not weapon TIERS** — the weapon family is now three tiers deep (`wpn_axe_{wood,stone,iron}_01`), so read this row as the `A`/`W`/`S` item triple it has always been, not as a tier list. **⚠ CORRECTED 2026-07-30 (`86caynyq7`): this line used to read "axe = shipped slate/steel FBX". "Slate/steel" described the RETIRED CC-BY hatchet (`Assets/Art/Props/CastawayAxe/`, deleted in PR #100 / `031d43a`); the shipped stone axe is a grey knapped biface with NO leather-wrapped grip (`weapon-tool-style-spec.md` §4.1: "NO lashing, NO rawhide, NO cord"). The DECISION is untouched — only the prop descriptor is re-anchored to the shipped mesh. Operative bake spec: [`item-icon-bake-recipe.md`](item-icon-bake-recipe.md) §2.1.** (§9's cross-reference line still reads "slate/steel" — that is a *citation of* DECISIONS 2026-06-14 and is accurate AS HISTORY, the same call Drew cleared for `item-icon-bake-recipe.md:112`. Leave it alone.) Atlas all icons into one **Sprite Atlas** (`Assets/Art/UI/ItemIcons.spriteatlasv2`) before the 4th distinct icon (Erik §E3 / research §E3 — keeps icon-bearing slots in one draw batch under the 8-texture cap). Fallback: a chunky warm-cream letter-chip (`A`/`W`/`S`) if a bake lags.
...
- DECISIONS 2026-06-14 — axe-head slate/steel ACCEPTED, barn-red DROPPED. **Cited AS HISTORY.** The decision binds the axe icon's palette *to the shipped prop* (not the board's red) — but the operative spec an icon-baker follows is [`item-icon-bake-recipe.md`](item-icon-bake-recipe.md) §2.1 (*"match the SHIPPED prop, do NOT recolor"*), which is exactly what §4.6 defers to; this bullet **records** the decision, it does not anchor §4.6. The `slate/steel` wording is the decision AS RECORDED — it described the retired hatchet (§4.6's `⚠ CORRECTED` note has the detail) — so **leave the text above intact** (`86caynyq7` constraint 3 · re-confirmed `86cayw6ve`).

> **Staleness-scan note (`86cayw6ve`).** Exactly **two LINES** in this file legitimately contain the RETIRED axe descriptor: §4.6's `⚠ CORRECTED` marker (which quotes the wording in order to retire it) and the DECISIONS citation directly above. **Two LINES is not two occurrences** — each of those lines carries the phrase more than once, and the guard is line-scoped, so the LINE count is the claim; conflating lines with occurrences overstates what the guard checked. A bare "zero hits" grep for that phrase can therefore NEVER pass, and must never be "satisfied" by striking either line — that would destroy the record the marker and the citation exist to keep. The keepable form matches the phrase **case-insensitively** and negates the marker context (`grep -n -i … | grep -v 'CORRECTED\|DECISIONS'` → **zero LINES**; judge the OUTPUT, not the exit code — a clean run exits 1). The `-i` is load-bearing *in this very file*: §4.6 already carries a sentence-start **capitalized** form of the descriptor, so a case-sensitive match would walk past a future live claim written in that same natural shape (measured: `lines=0` without `-i`, `lines=1` with it). The exclusion deliberately stays case-SENSITIVE — the marker words are an ALL-CAPS convention, and folding case into it too would silently exclude any live-claim line that merely contains lowercase prose "decisions". Both greps stay line-scoped, so a live claim placed ON either marker-bearing line still slips through: `-i` narrows that hole by one case, it does not close it. This note deliberately does not quote the phrase itself, so the count it states stays true. Exact command, the general rule, and the prove-the-RED requirement: [`team/TESTING_BAR.md`](../TESTING_BAR.md) § "Doc-staleness greps".
</content>
</invoke>
