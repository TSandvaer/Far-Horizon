...
- **Source prop:** the **SHIPPED in-hand axe** — `Assets/Art/Props/WeaponPack/wpn_axe_stone_01.fbx` (`WeaponPackAssetGen.AxeFbxPath`), the same FBX `MovementCameraScene` seats on the castaway's right-hand bone. Render the real held axe — do not model a new one. **⚠ CORRECTED 2026-07-28: this line used to name `CastawayAxe/`, which was deleted in PR #100 (`031d43a`, `86cabh907`) when the in-house weapon SET shipped. That path does not exist — do not bake from it.**
...
- **Palette — match the SHIPPED prop, do NOT recolor:** take the surface from the mesh you bake, i.e. the stone-tier recipe measured in [`weapon-tool-style-spec.md`](weapon-tool-style-spec.md) §4.1 (knapped grey biface + white cutting-arc band + straight brown haft with a dark grip band). **⚠ CORRECTED 2026-07-28: the previous wording here — "slate/steel head, warm-brown haft, dark leather-wrapped grip" — described the RETIRED CC-BY hatchet, whose leather-wrapped grip the shipped `wpn_axe_stone_01` does not have (§4.1: "NO lashing, NO rawhide, NO cord").** The principle is unchanged and is the load-bearing part: bake the real prop and do not recolor it. **Do NOT recolor the icon to the board's barn-red** (`21h08_08`'s red is the abstract style-guide ideal; the shipped accepted axe head is grey, not red — DECISIONS 2026-06-14: "axe-head ACCEPTED… genuinely looks like an axe"; barn-red recolor DROPPED, and the in-house stone axe that replaced it kept the grey). **Icon ⇄ in-hand consistency wins over the board ideal** — the player sees a grey-headed axe in his hand, so the slot must show a grey-headed axe. (This is the one place the icon deliberately diverges from the reference PNG, and it's the *right* divergence — coherence with the shipped prop beats fidelity to the board.)
...
- `Assets/Art/Props/WeaponPack/wpn_axe_stone_01.fbx` — the shipped in-house stone axe FBX (the AXE icon's render source; grey-knapped/off-white-edge per `weapon-tool-style-spec.md` §4.1, NOT barn-red per DECISIONS 2026-06-14). **Replaces `CastawayAxe/`, deleted in PR #100 (`031d43a`).**
...
- **Decision draft:** The AXE icon matches the **SHIPPED in-house stone axe** (`Assets/Art/Props/WeaponPack/wpn_axe_stone_01.fbx` — grey knapped biface + white cutting-arc band, per §2.1), NOT the board's barn-red `21h08_08` ideal — icon⇄in-hand coherence wins over board fidelity (extends DECISIONS 2026-06-14 axe-head call to the icon). **⚠ CORRECTED 2026-07-30: this draft used to read "the SHIPPED slate/steel hatchet" — that descriptor named the RETIRED CC-BY hatchet (`Assets/Art/Props/CastawayAxe/`, deleted in PR #100, `031d43a`), so it contradicted the §2.1 `:60` correction 60 lines above it. The DECISION is unchanged — only the prop descriptor is re-anchored to the shipped mesh.** (`:112`'s "slate/steel" wording is a *citation of* DECISIONS 2026-06-14 and is accurate AS HISTORY — leave it alone.)
