...
| 11 | A found item reads as a **warm discovery beat** — attract-cued enough to notice, never a loot-explosion or a beam of light. **CONFIRMED IN THIS SHAPE: a STILL, distinctively-silhouetted object in a deliberate placement** (FORM + POSITION, Bar 10's top two ranks), zero motion, no emissive/rim/beam. **MOTION is ruled OUT for anything held by a host** — it was tried and BROKE the read (a bobbing embedded item reads as *floating inside* its host at any amplitude; `game-juice.md` §1.5a). Corollary: **subtended size under-predicts legibility** — spotted unprompted at ~21 px of sword against a ~43 px stump, so do not spend a channel because a px estimate says it is too small. | world finds / discoveries, attract cues on placed items | soak PASS 2026-08-02 (`86cah7y5b` AC7(c), PR #351) — *"holding S for 4 seconds after spawn: yes the sword caught my eye"* at default framing, after the 2026-08-02 motion soak FAIL (*"the sword is floating, moving in the stump"*) |
