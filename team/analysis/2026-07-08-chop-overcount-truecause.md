...
**Why the shipped game is fine:** the default `chopInterval = 0.25s` (`ChopTree.cs:215`) inserts a stable window after each clip where SwingInProgress is false AND the cooldown blocks the next swing — the real cadence has the edge the test rig deleted. The `-verifyChop` windowed capture passing at real framerate corroborates.
