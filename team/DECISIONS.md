...
## 2026-06-17 — M-U2 survival loop EXPANDED from WARMTH-only to three needs (warmth + hunger + thirst)

- Decided by: Sponsor (2026-06-17, via /sponsor-questions-walkthrough; draft logged in `.claude/away-queue.md`; formalized here at gameplay-wave start)
- Decision: M-U2 is no longer the WARMTH-only thin loop (per the 2026-06-12 single-need decision). It now carries **three survival needs** drawn from the integrated vision doc: **WARMTH** (existing — campfire), **HUNGER** (eat berries harvested from bushes), **THIRST** (drink from a freshwater pond). Decomposed into tickets: hunger need `86caamkp8`, thirst need + pond `86caamkv7`, three-bar need HUD `86caamkxv`, plus the bushes/berries source `86caa5zz3`. CLAUDE.md M-U2 scope line + the vision-doc reference reconciled in this same PR. A cup/container to hold more water and a crafting table stay M-U3+ (explicitly OOS for this wave).
- Why: the vision arc wants the fuller survival loop; the Sponsor chose to have M-U2 carry hunger + thirst rather than defer them to a later milestone. The three needs share the WarmthNeed model surface (`Current01`/`Max`/`IsCritical`/`Changed`, Time.time-window decay, `TickSeconds`), so the marginal cost of the two new needs is low.
- Reversibility: reversible (additive need systems; per-need revert in ≤1 PR; the WARMTH-only loop still ships if hunger/thirst are dropped).
- Affects: M-U2 scope, survival-need model, SurvivalHud, world-gen (pond + bushes), settings registry (need tweakables), Devon + Drew + Tess + Priya (scope).
...
## 2026-06-20 — Sponsor return walkthrough: #103 merge, #82+#83 rebase, gameplay-wave next
- Decided by: Sponsor (2026-06-20, /sponsor-questions-walkthrough on return from the overnight away run)
- Decision: Three calls. (1) **MERGE #103** (Unity 6000.4.10f1→6000.4.11f1 upgrade) despite its value being re-scoped overnight — it does NOT fix the play-mode-enter hang (that's the separate Android-module ADB-scan issue `86caapwmt`); its real value is Drew's CI bootstrap-robustness fix (silent-green→fail-RED) + version currency. (2) **Rebase + merge BOTH #82 and #83** (force-push, Sponsor's hands, after the runner cache-clear) — un-gating the gameplay wave (#82) and the settings-console foundation (#83→Tess re-QA). (3) **Next focus = the GAMEPLAY WAVE** (thirst pond `86caamkv7`, chop `86caa4c5c`, stones `86caa4c96`, sticks `86caa96rd`) once the survival loop (#100 weapon set + #101 bushes) lands; the POC R&D lane (boat `86caa9zju` / big-island `86caa9zpp`) stays deferred.
- Why: (1) all #103 gates green + net-positive (the bootstrap fix already caught the #101 EPERM honestly) + trivially reverted, and 6000.4.11f1 is already installed locally + on the runner. (2) clears the rebase backlog and un-gates the next wave + the console in one pass. (3) continues the survival systems the merged hunger/warmth loop builds toward, over a higher-risk exploration pivot.
- Reversibility: reversible (revert the #103 squash; the rebases/merges revert in ≤1 PR each; the focus call is a priority order re-settable in one board edit).
- Affects: #103/#82/#83 merge flow, the gameplay-wave dispatch order, orchestrator + Devon + Drew + Tess + Priya.
