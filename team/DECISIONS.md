...
> **Correction protocol (established 2026-07-30, `86caz4td5`):** a **merged** entry is never rewritten. Append a new entry whose title begins `CORRECTION:` and which carries an extra **`- Amends:`** field quoting the amended entry's title verbatim + the SHA it merged in. Name the clause **withdrawn**, the clause that **replaces** it, and — explicitly — what still **stands**, so the original stays readable as the decision that WAS taken and a later reader can tell a narrow fix from a full reversal. Corrections are Priya's, same as entries. (An entry still sitting on an **unmerged** branch may be edited in place — that is a draft revision, not a correction, and needs none of this.)
...
- Amends: the entry **"An asset/hero PROVENANCE commit must include the export SCRIPT — `.blend` + FBX alone is not a reproducible export"** (dated 2026-07-30, below; merged in `51f4623`, PR #367). Correction by Priya — the amended entry is my own; `86caz4td5`.
...
- **Replaced by:** an export / provenance / asset-build script is committed to **`tools/debug/`**, with a one-line row in `tools/debug/REGISTRY.md`. `art-src/` holds **non-executable source artifacts only** — `.blend` sources, palette PNGs, concept art, hero/family renders, provenance `README`s.
- **Still stands, unchanged:** that the export RECIPE is *provenance* and binding on every asset/hero provenance commit; that it must record (1) the mandatory `Join` step, (2) the resulting joined object's NAME, (3) the axis/scale export settings; the load-bearing reason (Unity's **Generic** rig binds by transform path, so a differently-named `Join` imports cleanly and silently never binds); that a README statement of what was actually done is the acceptable minimum; and that `blender-asset-pipeline.md:279`'s *"next to the FBX"* was wrong. Only the replacement directory changes — this is a narrow location fix, not a reversal of the rule.
